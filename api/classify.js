...
function detectImageMediaType(buffer) {
  ...
  return null; // не распознали — используем то, что прислали в заголовке
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (contentType.startsWith("image/")) {
      ...
      existingTags = normalizeTags(tagsParam);

      const base64Data = raw.toString("base64"); // кодируем в Node — никаких хвостовых пробелов
      const detectedType = detectImageMediaType(raw);
      const actualMediaType = detectedType || contentType; // если не распознали — доверяем заголовку
      ...
      existingTags = normalizeTags(body.tags);
